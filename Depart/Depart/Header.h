...
private:
    ...
    struct sDataRecord {
        ...
        friend istream& operator>> (istream& in, sDataRecord& temp) {
            ...
            in.getline(temp.ID, 30);
            cout << "Please enter the Department Name ";
            in.getline(temp.Name, 50);
            cout << "Please enter the Department Manager ";
            in.getline(temp.Manager, 50);

            return  in;
        }
        friend ostream& operator<< (ostream& out, sDataRecord& obj) {
            out << "Department ID : " << obj.ID << endl;
            out << "Department Name : " << obj.Name << endl;
            out << "Department Manager : " << obj.Manager << endl;
    ...
    struct sSecondaryIndx {
        ...
        bool operator<(sSecondaryIndx& obj) {
            return (strcmp(DepartmentName, obj.DepartmentName) < 0);
        }
    };
    ...
    string mDataFilePath = "DataFile.txt";
    string mPrimaryIndxPath = "PrimaryIndex";
    string mSecondaryIndxFilePath = "SecondaryIndex";
    string mInvertedListFilePath = "InvertedList";
...
public:
    ...
    void WriteDataRecord(sDataRecord& temp) {
        ...
        strcat_s(Buffer, 50, temp.Manager); strcat_s(Buffer, 50, "|");
    ...
    void constructIndxForRecord(sDataRecord& obj, int offset) {
        ...
        int indxOfSecondary = BinarySearchSecondary(obj.Name);
        if (indxOfSecondary >= 0) {
            ...
            strcpy_s(tSecondary.DepartmentName, 50, obj.Name);
    ...
    void getRecordsByName(char* Name = 0) {
        if (Name == 0) {
            Name = new char[51];
            cout << "Please provide the Department Name  you want to search for : ";
            cin >> Name;
        }
...
//            tempSecondary.PrimaryIndx = i ; /// todo
    ...
    int BinarySearchSecondary(char* Name) {
        ...
        while (l <= r) {
            int mid = (l + r) / 2;
            if (strcmp(msSecondaryIndx[mid].DepartmentName, Name) == 0) {
                return mid;
            }
            else if (strcmp(msSecondaryIndx[mid].DepartmentName, Name) < 0) {
                l = mid + 1;
    ...
    void printIndxes() {
        ...
        for (auto it : msSecondaryIndx) {
            cout << it.DepartmentName << " " << it.InvertedListIndx << endl;
        }
        puts("---");
